...
# slightly modified to download win2k25 iso
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            win2025-eval)
                ...
                ;;
            all)
                media_list="$win7x64_ultimate $win81x64 $win10x64 $win11x64 $win81x64_enterprise_eval $win10x64_enterprise_eval $win11x64_enterprise_eval $win10x64_enterprise_ltsc_eval $win2008r2 $win2012r2_eval $win2016_eval $win2019_eval $win2022_eval $win2025_eval"
                break
                ;;
            *)
...
enterprise_eval_download() {
    ...
    # Get all fwlink URLs (works for all server versions)
    iso_download_links="$(echo "$iso_download_page_html" | grep -io "https://go.microsoft.com/fwlink.*linkid=[0-9]\+&clcid=0x[0-9a-z]\+&culture=en-us&country=us")"
    
    # Limit untrusted size for input validation
    iso_download_links="$(echo "$iso_download_links" | head -c 4096)"

    # Follow each redirect and find the English (US) x64 ISO
    for link in $iso_download_links; do
        # Follow redirect to get actual download URL
        actual_url="$(curl --location --output /dev/null --silent --write-out "%{url_effective}" --head --fail --proto =https --tlsv1.2 --http1.1 -- "$link" 2>/dev/null)"
        
        # Check if it's an English (US) x64 ISO (not VHD, not other languages)
        # Patterns: x64FRE_en-us.iso, _x64_en-us.iso, x64_English.iso, etc.
        if echo "$actual_url" | grep -qi "x64.*en-us\.iso$"; then
            iso_download_link="$actual_url"
            break
        elif echo "$actual_url" | grep -qi "x64.*_en-us\.iso$"; then
            iso_download_link="$actual_url"
            break
        elif echo "$actual_url" | grep -qi "x64fre.*en-us\.iso$"; then
            iso_download_link="$actual_url"
            break
        fi
    done
    ...
    if ! [ "$iso_download_link" ]; then
        echo_err "Could not find English (US) x64 ISO download link"
        return 1
    fi
...
enterprise_eval_download_old() {
    ...
    case "$enterprise_type" in
        ...
        "server25") iso_download_link=$(echo "$iso_download_links" | head -n 1 | tail -n 1) ;;
    ...
    for media in $media_list; do
        case "$media" in
            ...
            "$win2025_eval")
                ...
                enterprise_eval_download "$media" windows-server-2025 server25
                ;;
        esac || {
            error_action=$?
